...
/*   Updated: 2020/09/02 22:05:35 by casubmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	ft_get_values(t_all *all, t_raycast *r, int line_height)
{
	...
	all->print.tex_pos = (r->drawStart - all->window.screen_h / 2 + line_height / 2) * all->print.step;
...
static	void	ft_get_textur_width_height(t_all *all)
{
	...
	if (r->side == 2 || r->side== 3)
		all->print.wall_x = all->player.y + r->perp_wall_dist * r->ray_dir_y;
	else
		all->print.wall_x = all->player.x + r->perp_wall_dist * r->ray_dir_x;
}
...
void			ft_print(t_all *all, int line_height, int x_pix)
{
	...
	while (all->print.y < r->drawEnd)
	{
		...
		if (r->side == 0)
			all->print.color = ft_get_pixel_color(&(all->texturs.so), all->print.text_x, all->print.text_y);
		else if (r->side == 1)
			all->print.color = ft_get_pixel_color(&(all->texturs.no), all->print.text_x, all->print.text_y);
		else if (r->side == 2)
			all->print.color = ft_get_pixel_color(&(all->texturs.ea), all->print.text_x, all->print.text_y);
		else if (r->side == 3)
			all->print.color = ft_get_pixel_color(&(all->texturs.we), all->print.text_x, all->print.text_y);
