...
/*   Updated: 2020/09/02 22:22:55 by casubmar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_init_textures(t_all *all)
{
	...
	"pic1.xpm", &(all->texturs.so.width), &(all->texturs.so.height))))
		return (1);
	if (!(all->texturs.no.img = mlx_xpm_file_to_image(all->window.mlx,\
	"pic2.xpm", &(all->texturs.no.width), &(all->texturs.no.height))))
		return (1);
	if (!(all->texturs.ea.img = mlx_xpm_file_to_image(all->window.mlx,\
	"pic3.xpm", &(all->texturs.ea.width), &(all->texturs.ea.height))))
		return (1);
	if (!(all->texturs.we.img = mlx_xpm_file_to_image(all->window.mlx,\
	"pic1.xpm", &(all->texturs.we.width), &(all->texturs.we.height))))
		return (1);
	if (!(all->sprite.data.img = mlx_xpm_file_to_image(all->window.mlx,\
	"pillar.xpm", &(all->sprite.data.width), &(all->sprite.data.height))))
		return (1);
